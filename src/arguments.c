...
char *get_default_database_path(TYPE type) {
    char *path = getenv((type == TYPE_directories) ? directories_env_variable : files_env_variable);
    if (path == NULL) {
        ...
        strcpy(path, home);
        strcat(path, (type == TYPE_directories ) ? default_dir_database : default_files_database);
    }
    return path;
}
