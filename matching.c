...
static const int first_gap_penalty = 8;
static const int gap_penalty = 1;
static const int start_bonus = 3;
...
static int *matching_bonus(const char *string, int n) {
  ...
  int last_slash = -1;
  bonus[0] = match_bonus + start_bonus;

  for (int i = 1; i < n; i++) {
    if (is_separator(string[i - 1]) && isalpha(string[i])) {
      bonus[i] = match_bonus + start_bonus;
    } else {
      bonus[i] = match_bonus;
    }
    if (string[i] == '/') {
...
static MatchingData *make_data(const char *string, const char *query) {
  ...
  *data = (MatchingData){
      ...
      .query = query,
      .matrix = matrix,
      .bonus = matching_bonus(string, n - 1),
  };
  return data;
}
